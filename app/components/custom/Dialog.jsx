...
export function DialogDescription({ className, ...props }) {
  return <HeadlessDialog.Description as={Text} {...props} className={clsx(className, 'mt-2 text-pretty')} />
}
...
export function DialogActions({ className, ...props }) {
  return (
    <div
      ...
      className={clsx(
        className,
        'mt-8 flex flex-row items-center justify-end gap-3 *:w-full sm:flex-row sm:*:w-auto'
      )}
    />
  )
}
